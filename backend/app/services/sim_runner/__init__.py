...
from .state import RunnerStatus, AgentAction, RoundSummary, SimulationRunState
...
class SimulationRunner(HeartbeatMixin, SubprocessMixin):
    ...
    def get_run_state(cls, simulation_id: str) -> Optional[SimulationRunState]:
        """Get simulation run state (from memory or file)."""
    ...
    def stop_simulation(cls, simulation_id: str) -> SimulationRunState:
        ...
        if process and process.poll() is None:
            try:
                ...
                logger.error(f"Failed to terminate process: {simulation_id}, error={e}")
                try:
                    process.terminate()
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        """Clean up simulation log files for a fresh restart."""
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        ...
        logger.info(f"Cleaned simulation logs: {simulation_id}, files: {cleaned_files}")
        return {
    ...
    def cleanup_all_simulations(cls):
        """Clean up all running simulation processes (called on server shutdown)."""
        ...
        for simulation_id, process in processes:
            try:
                ...
                logger.error(f"Failed to clean up simulation: {simulation_id}, error={e}")
        ...
        for file_handle in list(cls._stderr_files.values()):
            try:
                if file_handle:
                    file_handle.close()
            except Exception:
    ...
    def _read_actions_from_file(
        ...
        """Read actions from a single actions file."""
    ...
    def get_all_actions(
        ...
        """Get full action history (no pagination limit)."""
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        actions = []
        ...
        if not platform or platform == "twitter":
            actions.extend(cls._read_actions_from_file(
                twitter_actions_log, default_platform="twitter",
        ...
        if not platform or platform == "reddit":
            actions.extend(cls._read_actions_from_file(
                reddit_actions_log, default_platform="reddit",
                platform_filter=platform, agent_id=agent_id, round_num=round_num,
            ))
        ...
        if not actions:
            ...
            actions = cls._read_actions_from_file(
                actions_log, default_platform=None,
                platform_filter=platform, agent_id=agent_id, round_num=round_num,
            )
    ...
    def get_agent_stats(cls, simulation_id: str) -> List[Dict[str, Any]]:
        ...
        for action in actions:
            ...
            if action.platform == "twitter":
                ...
                s["reddit_actions"] += 1
            s["action_types"][action.action_type] = s["action_types"].get(action.action_type, 0) + 1
    ...
    def get_running_simulations(cls) -> List[str]:
        """Get all running simulation IDs."""
        running = []
    ...
    def get_env_status_detail(cls, simulation_id: str) -> Dict[str, Any]:
        ...
        default = {"status": "stopped", "twitter_available": False, "reddit_available": False, "timestamp": None}
    ...
    def interview_agent(
        ...
        if response.status.value == "completed":
            return {"success": True, "agent_id": agent_id, "prompt": prompt, "result": response.result, "timestamp": response.timestamp}
        return {"success": False, "agent_id": agent_id, "prompt": prompt, "error": response.error, "timestamp": response.timestamp}
    ...
    def interview_agents_batch(
        ...
        if response.status.value == "completed":
            return {"success": True, "interviews_count": len(interviews), "result": response.result, "timestamp": response.timestamp}
        return {"success": False, "interviews_count": len(interviews), "error": response.error, "timestamp": response.timestamp}
    ...
    def interview_all_agents(
        ...
        for ac in agent_configs:
            aid = ac.get("agent_id")
            if aid is not None:
                interviews.append({"agent_id": aid, "prompt": prompt})

        return cls.interview_agents_batch(simulation_id=simulation_id, interviews=interviews, platform=platform, timeout=timeout)
    ...
    def close_simulation_env(cls, simulation_id: str, timeout: float = 30.0) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": response.status.value == "completed", "message": "Close env command sent", "result": response.result, "timestamp": response.timestamp}
        except TimeoutError:
            return {"success": True, "message": "Close env command sent (timeout waiting for response)"}
    ...
    def get_interview_history(
        ...
        """Get interview history from database(s)."""
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        ...
        for p in platforms:
            ...
            results.extend(cls._get_interview_history_from_db(
