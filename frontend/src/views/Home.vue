<template>
  <div class="home-container">
    ...
    <Transition name="splash-fade">
      <div v-if="showWelcomeSplash" class="splash-overlay">
        <div class="splash-ambient top-left" />
        <div class="splash-ambient bottom-right" />
        ...
        <div class="splash-card">
          ...
          <div class="splash-header">
            <div class="splash-icon-wrap">
              ...
              <div class="splash-icon-box">
                <svg viewBox="0 0 100 100" class="splash-icon" fill="currentColor">
                  <rect x="48" y="20" width="4" height="60" rx="2" />
                  <circle cx="50" cy="18" r="4" />
                  <path d="M50 35 C15 22, 10 32, 50 35 Z" fillOpacity="0.8" />
                  <path d="M50 35 C85 22, 90 32, 50 35 Z" fillOpacity="0.8" />
                  <path d="M50 45 C20 40, 18 48, 50 45 Z" fillOpacity="0.6" />
                  <path d="M50 45 C80 40, 82 48, 50 45 Z" fillOpacity="0.6" />
                </svg>
              </div>
            </div>

            <span class="splash-badge">MULTI-AGENT SOCIAL RESEARCH PLATFORM</span>
            <h1 class="splash-title">KINJENG PROJECT</h1>
            <div class="splash-divider" />
            <p class="splash-desc">
              Ekosistem pemodelan sosiokultural berbasis kognisi agen kecerdasan buatan. Mengombinasikan graph pengetahuan interaktif dengan mesin rujukan sosiologis untuk menyajikan simulasi data sosial se-presisi mungkin.
            </p>
          </div>
          ...
          <div class="splash-features">
            <div class="sf-item">
              ...
              <div>
                <span class="sf-label">Engine MiroFish V1.0</span>
                <span class="sf-sub">Model simulasi kognitif agen sosiokultural.</span>
              </div>
            </div>
            <div class="sf-item">
              <span class="sf-dot">■</span>
              <div>
                <span class="sf-label">Graph Interaksi</span>
                <span class="sf-sub">Visualisasi riil jaringan korelasi komunikasi.</span>
              </div>
            </div>
            <div class="sf-item">
              <span class="sf-dot">■</span>
              <div>
                <span class="sf-label">Consensus Debating</span>
                <span class="sf-sub">Diskursus internal tandingan multi-perspektif.</span>
              </div>
            </div>
            <div class="sf-item">
              <span class="sf-dot">■</span>
              <div>
                <span class="sf-label">Tabulasi Analitik</span>
                <span class="sf-sub">Statistik distribusi frekuensi & tabulasi silang.</span>
              </div>
            </div>
          </div>

          <button class="splash-btn" @click="dismissSplash">
            <span>MULAI EKSPLORASI DASBOR</span>
    ...
    <Transition name="toast-fade">
      <div v-if="toastMessage" class="splash-toast">{{ toastMessage }}</div>
    </Transition>
...
function dismissSplash() {
  ...
  showWelcomeSplash.value = false
  toastMessage.value = 'Selamat Datang di Kinjeng Project!'
...
.splash-ambient.top-left { top: 25%; left: 25%; }
.splash-ambient.bottom-right { bottom: 25%; right: 25%; opacity: 0.05; }
.splash-card {
  max-width: 650px;
  width: 100%;
  background: #0a0a0a;
  ...
  position: relative;
  box-shadow: 0 0 50px rgba(255,106,51,0.05);
  margin: auto;
}
.corner-ornament {
  position: absolute;
...
.corner-ornament.top-left { top: -1px; left: -1px; border-top: 2px solid #FF6A33; border-left: 2px solid #FF6A33; }
.corner-ornament.top-right { top: -1px; right: -1px; border-top: 2px solid #FF6A33; border-right: 2px solid #FF6A33; }
.corner-ornament.bottom-left { bottom: -1px; left: -1px; border-bottom: 2px solid #FF6A33; border-left: 2px solid #FF6A33; }
.corner-ornament.bottom-right { bottom: -1px; right: -1px; border-bottom: 2px solid #FF6A33; border-right: 2px solid #FF6A33; }
.splash-header { display: flex; flex-direction: column; align-items: center; margin-bottom: 20px; }
.splash-icon-wrap { position: relative; margin-bottom: 14px; display: flex; align-items: center; justify-content: center; }
.splash-pulse {
  position: absolute;
  inset: 0;
  border-radius: 50%;
  background: #FF6A33;
  opacity: 0.05;
  animation: splashPing 2s ease-in-out infinite;
}
@keyframes splashPing {
  0%, 100% { transform: scale(1); opacity: 0.05; }
  50% { transform: scale(1.4); opacity: 0.02; }
}
.splash-icon-box {
  width: 64px;
  height: 64px;
...
.splash-icon { width: 40px; height: 40px; color: #FF6A33; }
.splash-badge {
  font-family: var(--font-mono);
  font-size: 9px;
...
@media (min-width: 768px) { .splash-title { font-size: 2.5rem; } }
.splash-divider {
  width: 96px;
  height: 1px;
  background: #262626;
...
@media (min-width: 640px) { .splash-features { grid-template-columns: 1fr 1fr; } }
.sf-item { display: flex; align-items: flex-start; gap: 8px; }
.sf-dot { color: #FF6A33; margin-top: 2px; font-size: 0.7rem; }
.sf-label { color: #fff; font-weight: 700; display: block; }
.sf-sub { color: #737373; font-size: 10px; }
.splash-btn {
  width: 100%;
  display: inline-flex;
  align-items: center;
...
.splash-btn:hover { background: #070707; outline: 1px solid #FF6A33; }
.splash-btn-arrow { transition: transform 0.2s; }
...
.splash-fade-enter-active { transition: opacity 0.3s ease-in-out; }
.splash-fade-leave-active { transition: opacity 0.3s ease-in-out; }
