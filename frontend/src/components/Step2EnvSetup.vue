    ...
    <div class="scroll-container">
        ...
        <div class="card-content">
          ...
          <div v-if="profiles.length > 0" class="stats-grid">
            <div class="stat-card">
              ...
              <span v-if="validationErrors.agentCount" class="field-error">{{ validationErrors.agentCount }}</span>
            </div>
            <div class="stat-card">
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <!-- Informasi round (HANYA INFORMASI, tidak bisa diubah) -->
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section info-only">
          ...
          <div class="action-group dual">
            ...
            <div class="rounds-validation">
              <span v-if="validationErrors.rounds" class="field-error">{{ validationErrors.rounds }}</span>
            </div>
            <button 
...
const props = defineProps({
  ...
  graphData: Object,
  systemLogs: Array,
  // initialRounds removed — round count is now auto-calculated, info-only in Step 2
})
...
function validateForm() {
  const errors = {}
  const ac = profiles.value.length || 0
  if (ac < 1) errors.agentCount = 'Jumlah agent minimal 1'
  else if (ac > 10000) errors.agentCount = 'Jumlah agent maksimal 10.000'
  
  const rounds = autoGeneratedRounds.value
  if (rounds < 1) errors.rounds = 'Jumlah ronde minimal 1'
  else if (rounds > 1000) errors.rounds = 'Jumlah ronde maksimal 1000'
  
  validationErrors.value = errors
  const valid = Object.keys(errors).length === 0
  if (!valid) {
    addLog(t('log.validationFailed') + ': ' + Object.values(errors).join(', '))
  }
  return valid
}
...
// Round count is auto-calculated from simulation config, displayed as info only
...
const autoGeneratedRounds = computed(() => {
  ...
  const totalHours = cfg.time_config.total_simulation_hours
  const minutesPerRound = cfg.time_config.minutes_per_round
  if (!totalHours || !minutesPerRound) {
    return 40
  }
  const calculatedRounds = Math.floor((totalHours * 60) / minutesPerRound)
  return Math.min(Math.max(calculatedRounds, 40), 1000)
})
...
const handleStartSimulation = () => {
  ...
  emit('next-step', { maxRounds: autoGeneratedRounds.value })
}
...
.badge.processing { background: #FF5722; color: var(--bg-card); }
